	...
	public function index() {
		...
		if ($this->request->is('post')) {
			...
			if ($this->Seller->validates($this->request->data)) {
				...
				if (!in_array($data['file']['type'], $mimetypes)) {
				...
				else if (isset($data['file']['error']) && $data['file']['error'] == 0) {
					...
					if ($sellers == false) { // 読み込みが失敗した。
						...
						//　セッションにて保管する。
						$this->Session->write('sellers', array('price'=> $data['price'], 'sellers'=> $sellers));
					}
					unlink($data['file']['tmp_name']);
				} else {
					$this->Session->setFlash(__('アップロードできませんでした。'), 'default', array(), 'upload');
				}
			}
		}
		// 出品者一覧を読み込み
		if (($sellers = $this->Session->read('sellers')) != null) {
			$this->set('price', $sellers['price']);
